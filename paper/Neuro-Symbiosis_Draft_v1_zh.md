...
### 3.4 能耗耦合机制假设
为使本文从“工程可运行”迈向“机制可检验”，我们引入一个可证伪的中心假设：在 SNN-Transformer 混合解码中，SNN 发放率与进入 Transformer 的有效 token 长度存在非线性耦合，而该耦合主导端到端能耗变化。

记平均发放率为 $r$，SNN 时间窗为 $T_s$，Transformer 实际参与计算的有效 token 长度为 $L$，嵌入维度为 $d$，则单样本总能耗近似写为
$$
E_{total}=E_{snn}(r,T_s)+E_{attn}(L,d)+E_{ffn}(L,d)+E_{io}.
$$
进一步假设
$$
L=g(r,T_s,\tau),
$$
其中 $\tau$ 表示阈值、token 化与稀疏控制超参数。直观上，较低的 $r$ 可压缩冗余时序激活，降低后续注意力计算；但过高或过低的脉冲活动都可能导致表征质量下降或 token 冗余增加。基于该假设，本文后续不只报告精度和能耗，还显式记录 $r$ 与 $L$，并考察二者对能耗的解释力。
...
### 5.6 耦合曲线与 LOSO 试点结果（新增）
为初步检验第 3.4 节的机制假设，本文新增两组结果：一组为基于 $(\beta, \theta)$ 网格的耦合曲线扫描，另一组为 LOSO 受试者留一验证试点。

1. 耦合曲线扫描：
在固定输入维度下，对 LIF 参数 $(\beta, \theta)$ 做 6×6 网格扫描，记录每组配置的 $r$、$L$ 与能耗代理值。结果显示：
- $corr(L, E)=+0.9905$
- $corr(r, E)=+0.7334$
- $corr(r\times L, E)=+0.7407$

这表明在当前 quick 设定下，能耗与有效 token 长度的相关性显著高于仅由发放率解释的相关性，说明 Transformer 侧的序列长度成本在总能耗中占主导。换言之，单纯降低脉冲率并不足以显著降耗，关键在于是否真正压缩了进入注意力模块的有效序列长度。

2. LOSO 试点：
基于 9 个受试者的合成 BCI 风格数据，使用 LOSO 协议对 Hybrid、SNN 和 Transformer 进行 9 折验证。汇总结果如下：

| 模型 | folds | val_acc(mean±std) | 95% CI | effective token length |
|---|---:|---:|---:|---:|
| Hybrid | 9 | 0.2901±0.0166 | 0.0109 | 127.8±0.0 |
| SNN | 9 | 0.2986±0.0176 | 0.0115 | 127.8±0.0 |
| Transformer | 9 | 0.2986±0.0217 | 0.0142 | 0.0±0.0 |

由于当前数据为随机合成的跨受试者试点，准确率接近四分类随机基线是符合预期的；其价值不在于绝对性能，而在于验证 LOSO 协议、指标采集链路与跨折统计流程均已跑通。更重要的是，Hybrid 与 SNN 在当前阈值下的 $L\approx128$，说明几乎所有时间步均被保留为有效 token。这直接提示下一阶段应通过阈值控制、时间窗裁剪或 token 压缩机制主动拉低 $L$，否则混合模型的注意力成本无法得到实质削减。

3. 对机制假设的初步判断：
现有结果支持“$L$ 比 $r$ 更接近总能耗主导因子”的判断，但尚未达到严格机制证明标准。要形成可发表证据，还需要在真实 BCI 数据、LOSO 设置和多随机种子条件下重复该趋势，并进一步验证在固定准确率带宽内是否存在能耗最优工作区间 $r^*$。
